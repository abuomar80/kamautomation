...
def create_manual_charge(charge_data, okapi, headers):
    """
    Creates or updates a manual charge
    :param charge_data: Dictionary containing manual charge information
    :param okapi: Okapi URL
    :param headers: Request headers
    :return: Tuple (success: bool, error_msg: str or None)
    """
    try:
        url = f"{okapi}/manual-charges"
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                # Check if manual charge exists by getting existing manual charges
                get_response = requests.get(f"{okapi}/manual-charges", headers=headers)
                if get_response.status_code == 200:
                    existing_charges = get_response.json().get('manualCharges', [])
                    # Try to find matching manual charge by nameReason
                    name_reason = charge_data.get('nameReason')
                    charge_id = charge_data.get('id')
                    
                    if charge_id:
                        # Update existing manual charge by ID
                        update_url = f"{okapi}/manual-charges/{charge_id}"
                        update_response = requests.put(update_url, json=charge_data, headers=headers)
                        if update_response.status_code in [200, 204]:
                            return True, None
                    else:
                        # Find by nameReason
                        for charge in existing_charges:
                            if str(charge.get('nameReason', '')) == str(name_reason):
                                charge_data['id'] = charge.get('id')
                                update_url = f"{okapi}/manual-charges/{charge.get('id')}"
                                update_response = requests.put(update_url, json=charge_data, headers=headers)
                                if update_response.status_code in [200, 204]:
                                    return True, None
                                break
                # If update fails or manual charge doesn't exist, return error
                error_data = response.json()
                error_msg = error_data.get('errors', [{}])[0].get('message', 'Manual charge already exists or validation failed')
            ...
            try:
                ...
                error_msg = f'Error creating manual charge (Status: {response.status_code})'
            return False, error_msg
...
def manual_charges():
    """
    Main function to create manual charges
    Each row in the Excel sheet will create one manual charge record
    """
    ...
    df = upload('ManualCharges')
    
    if df is None or df.empty:
        st.warning("Please upload a file with ManualCharges sheet.")
        return
    ...
    if bool(selected_rows):
        ...
        # Process each manual charge (loop through all rows)
        create_button = st.button('Create Manual Charges', type='primary')
        
        if create_button:
            error_messages = []
            success_count = 0
            
            with st.spinner('Creating manual charges...'):
                # Loop through each row and create a manual charge record
                for idx, row in selection.iterrows():
                    # Build manual charge data
                    charge_data = {
                    ...
                    if 'id' in row and pd.notna(row.get('id')) and str(row['id']).strip():
                        charge_data['id'] = str(row['id']).strip()
                    
                    # Create the manual charge (one API call per row)
                    success, error_msg = create_manual_charge(charge_data, okapi, headers)
                    
                    if success:
                        success_count += 1
                    else:
                        error_messages.append(f"Manual charge '{row.get('nameReason', 'N/A')}': {error_msg}")
                        st.warning(f"⚠️ Manual charge '{row.get('nameReason', 'N/A')}': {error_msg}")
            
            # Summary message
            if error_messages:
                st.success(f"✅ {success_count} manual charge(s) created successfully!")
                st.info(f"💡 {len(error_messages)} error(s) occurred - see warnings above for details.")
            else:
                st.success(f"✅ {success_count} manual charge(s) created successfully!")
                st.info("💡 All manual charges were created successfully.")
    else:
        st.warning('Please Select Required Manual Charges!!')
