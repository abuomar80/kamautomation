...
# Custom progress columns (replacement for folio_data_import._progress)
from rich.progress import ProgressColumn
from rich.text import Text

class ItemsPerSecondColumn(ProgressColumn):
    """Shows items/second processing speed"""
    def render(self, task):
        if task.speed is None:
            return Text("?", style="progress.data.speed")
        return Text(f"{task.speed:.1f}/s", style="progress.data.speed")

class UserStatsColumn(ProgressColumn):
    """Shows user stats: created/updated/failed"""
    def render(self, task):
        if hasattr(task, 'created') and hasattr(task, 'updated') and hasattr(task, 'failed'):
            return Text(
                f"C:{task.created} U:{task.updated} F:{task.failed}",
                style="progress.stats"
            )
        return Text("", style="progress.stats")
...
class UserImporter:  # noqa: R0902
    ...
    def __init__(
        ...
        gateway_url: str = None,  # Store gateway_url separately since FolioClient doesn't expose it
    ) -> None:
        ...
        # Store gateway_url - try to get it from folio_client if available, otherwise use provided value
        if gateway_url:
            self.gateway_url = gateway_url
        elif hasattr(folio_client, 'gateway_url'):
            self.gateway_url = folio_client.gateway_url
        elif hasattr(folio_client, 'okapi_url'):
            self.gateway_url = folio_client.okapi_url
        else:
            # Fallback: try to get it from okapi_headers or raise error
            raise ValueError("gateway_url must be provided. FolioClient doesn't expose this attribute.")
        ...
        # Track individual user results for CSV export
        self.user_results: List[dict] = []  # List of dicts: {username, status, action, timestamp, row_number, error}
    ...
    def get_results_csv(self) -> str:
        """
        Export user results to CSV format.
        ...
        # Write header
        writer.writerow(["Row Number", "Username", "Barcode", "Name", "Email", "Status", "Action", "Timestamp", "Error"])
        
        # Write data rows (sorted by row number)
        for result in sorted(self.user_results, key=lambda x: x["row_number"]):
            writer.writerow([
                result["row_number"],
                result["username"],
                result.get("barcode", ""),
    ...
    async def update_existing_user(
        ...
        create_update_user = await self.http_client.put(
            self.gateway_url + f"/users/{existing_user['id']}",
            headers=self.folio_client.okapi_headers,
            json=existing_user,
        )
    ...
    async def create_or_update_user(
        ...
        """
        username = user_obj.get("username", user_obj.get("externalSystemId", f"Row_{line_number}"))
        barcode = user_obj.get("barcode", "")
        personal = user_obj.get("personal", {})
        first_name = personal.get("firstName", "") if isinstance(personal, dict) else ""
        last_name = personal.get("lastName", "") if isinstance(personal, dict) else ""
        ...
        timestamp = dt.now().isoformat(sep=' ', timespec='milliseconds')
        ...
        if existing_user:
            ...
            try:
                ...
                error_msg = str(getattr(getattr(ee, 'response', str(ee)), 'text', str(ee)))
                logger.error(
                    f"Row {line_number}: User update failed: {error_msg}\n"
                )
            ...
            try:
                ...
                error_msg = str(getattr(getattr(ee, 'response', str(ee)), 'text', str(ee)))
                logger.error(
                    f"Row {line_number}: User creation failed: {error_msg}\n"
                )
                await self.errorfile.write(
    ...
    async def get_existing_spu(self, existing_user):
        ...
        try:
            existing_spu = await self.http_client.get(
                self.gateway_url + "/service-points-users",
                headers=self.folio_client.okapi_headers,
...
def main(
    ...
    try:
        importer = UserImporter(
            ...
            gateway_url=gateway_url  # Pass gateway_url separately since FolioClient doesn't expose it
        )
